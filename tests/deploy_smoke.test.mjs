...
test('runDeploySmoke reports stale deployments', async () => {
  const responses = new Map([
    ...
    ['https://example.test/map/js/transport-state.js', response(404, 'not found')],
  ...
  assert.deepEqual(
    ...
    ['module app script', 'js/config.js', 'js/app.js', 'js/data-normalization.js', 'js/portal-state.js', 'js/transport-state.js', 'js/search-index.js', 'js/chunk-label-state.js', 'js/search-focus-state.js', 'js/layer-state.js', 'js/monster-filter-state.js', 'js/url-state.js']
  );
});
