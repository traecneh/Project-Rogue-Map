...
test('runDeploySmoke checks index, config, app, and map image assets', async () => {
  const responses = new Map([
    ...
    ['https://example.test/map/js/app.js', response(200, "import { DATA } from './config.js'; import { buildSearchIndex } from './search-index.js'; import { chunkMonsterNames } from './chunk-label-state.js'; import { searchTypeForRun } from './search-focus-state.js'; import { normalizeTownList } from './data-normalization.js'; import { splitPortalItems } from './portal-state.js'; import { searchLabelMarkerState } from './layer-state.js'; import { monsterFilterStatusText } from './monster-filter-state.js'; import { urlWithSearchTerm } from './url-state.js';")],
...
test('runDeploySmoke reports stale deployments', async () => {
  const responses = new Map([
    ...
    ['https://example.test/map/js/portal-state.js', response(404, 'not found')],
  ...
  assert.deepEqual(
    ...
    ['module app script', 'js/config.js', 'js/app.js', 'js/data-normalization.js', 'js/portal-state.js', 'js/search-index.js', 'js/chunk-label-state.js', 'js/search-focus-state.js', 'js/layer-state.js', 'js/monster-filter-state.js', 'js/url-state.js']
  );
});
